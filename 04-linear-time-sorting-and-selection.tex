...
\title{04. Linear-Time Sorting and Selection}
\subtitle{CPSC 535 $\sim$ Spring 2019}
\author{Kevin A. Wortman}
\institute{ \includegraphics[height=2cm]{csuf-logo-cmyk} }
\date{February 18, 2019 \stanza
...
\textbf{Tool to circumvent} lower pounds, $NP$-hardness
\end{frame}
...
\textbf{output:} a permutation (reordering) $\langle a_1', a_2', \ldots, a_n' \rangle$
  of the input sequence such that $a_1' \leq a_2' \leq \ldots, a_n' \rangle.$ \stanza
...
\begin{column}{0.5\textwidth}
  \begin{algorithmic}[1]
    \Function{RPART}{A}
    \State $x = \text{rand. el't in } A[p \ldots r]$
    \State $i = p-1$
    \For { $j$ from $p$ to $r-1$ }
      \If { $A[j] \leq x$ }
        \State $i++$
