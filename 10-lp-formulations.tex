...
\begin{frame} \frametitle{Linear Programming Formulations}
  Many problems reduce to linear programming (LP):
  \begin{itemize}
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item unbounded: never happens because the
      $2.9I + 3.2O + .5R + .3L \leq 1400$
      constraint upper-bounds the objective function
...
\begin{enumerate}
  ...
  \item \textbf{Interpreting result:}
  \begin{itemize}
    \item infeasible: never happens because setting each $f_{u, v}=0$ is a feasible solution
    \item unbounded: never happens because the objective function is
    \[ \sum_{v \in V} f_{s, v} - \sum_{v, s} f_{v, s}, \]
     every $f_{s, v} \leq c(s, v)$ which is finite, and every $f_{v, s} \geq 0$
